...
func LoadTags(ctx context.Context, audio *models.Audio, r models.AudioReader) error {
	if err := audio.LoadTagIDs(ctx, r); err != nil {
		return fmt.Errorf("failed to load tag IDs for audio %d: %w", audio.ID, err)
	}

	return nil
}

func LoadPerformers(ctx context.Context, audio *models.Audio, r models.AudioReader) error {
	if err := audio.LoadPerformerIDs(ctx, r); err != nil {
		return fmt.Errorf("failed to load performer IDs for audio %d: %w", audio.ID, err)
	}
