...
| Audio marker cards collapse into a narrow strip | `AudioMarkersPanel.tsx:65` renders `.audio-markers-panel`, but the sizing rules live at `Scenes/styles.scss:604-608` under `.scene-markers-panel .wall .wall-item` (`min-height: 14rem; width: calc(100% - 2rem)`). No `.audio-markers-panel` or `.audio-marker-wall` rule has ever existed | User-visible on the Markers tab of every audio: cards render as a thin vertical strip with the title wrapped one character per line. Pre-existing — audio was copied from scene without the matching stylesheet, not caused by the wall removal (that block was scoped to `.audio-wall`, a different selector) |
