...
| 17 | `c637b2931` | `data-action` attributes (#6977) | **Nothing to port — audio already had it.** The hook reaches audio through the shared `CountButton`, which upstream patched directly and which arrived with the merge; `AudioCard` and `Audio.tsx` both import `OCounterButton` from there. Verified in the browser: `[data-action="o-counter"]` is present on the audio list and detail pages. Commit `44bb9ce1c` edited `Audios/AudioDetails/OCounterButton.tsx`, but that file had no importers and was a dead copy — deleted in `871263555` | ➖ |
...
| ~~`/audios/markers` crashed on load~~ | ~~`setObjectFilter` (`LabeledIdFilter.tsx`) had no `FilterMode.AudioMarkers` case and threw "Invalid filter mode"; `AudioMarkerList` renders `SidebarTagsFilter`, which calls it on mount~~ | **Resolved** in commit `77031fcac`. The whole page rendered as an error boundary. Audio markers have no `audio_markers_filter`/`audio_marker_count` in the schema, so the case falls back to `audio_count`. Another instance of the FilterMode-enumeration trap — grep every `FilterMode.SceneMarkers` switch when adding audio surfaces |
