...
export const AudioHistoryPanel: React.FC<IAudioHistoryProps> = ({ audio }) => {
  ...
  const [resetResume] = useAudioResetActivity(audio.id);
  const [resetDuration] = useAudioResetActivity(audio.id);
  ...
  async function handleResetResume() {
    try {
      // NOTE: the audio backend's resetActivity resolver (unlike the scene
      // equivalent) always resets both resume time and duration together —
      // it has no reset_resume/reset_duration parameters. This pre-existing
      // backend gap means this "reset resume" action currently also resets
      // duration; it is not something this frontend-only merge task can fix.
      await resetResume({
        variables: {
          id: audio.id,
        },
  ...
  async function handleResetDuration() {
    try {
      // See NOTE in handleResetResume above — backend resets both fields.
      await resetDuration({
        variables: {
          id: audio.id,
        },
