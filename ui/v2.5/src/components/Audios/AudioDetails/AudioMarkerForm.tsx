...
import {
  useAudioMarkerCreate,
  useAudioMarkerUpdate,
  useAudioMarkerDestroy,
} from "src/core/StashService";
import { DurationInput } from "src/components/Shared/DurationInput";
import { MarkerTitleSuggest } from "src/components/Shared/Select";
import { getPlayerPosition } from "src/components/AudioPlayer/util";
...
interface IAudioMarkerForm {
  audioID: string;
  marker?: GQL.AudioMarkerDataFragment;
  onClose: () => void;
}

export const AudioMarkerForm: React.FC<IAudioMarkerForm> = ({
  audioID,
  marker,
  onClose,
}) => {
  ...
  const [audioMarkerCreate] = useAudioMarkerCreate();
  const [audioMarkerUpdate] = useAudioMarkerUpdate();
  const [audioMarkerDestroy] = useAudioMarkerDestroy();
  ...
  async function onDelete() {
    ...
    try {
      await audioMarkerDestroy({ variables: { id: marker.id } });
    } catch (e) {
      Toast.error(e);
    } finally {
      onClose();
