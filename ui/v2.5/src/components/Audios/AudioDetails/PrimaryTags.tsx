...
interface IPrimaryTags {
  audioMarkers: GQL.AudioMarkerDataFragment[];
  onClickMarker: (marker: GQL.AudioMarkerDataFragment) => void;
  onEdit: (marker: GQL.AudioMarkerDataFragment) => void;
}
...
export const PrimaryTags: React.FC<IPrimaryTags> = ({
  ...
  if (!audioMarkers?.length) return <div />;

  const primaryTagNames: Record<string, string> = {};
  const markersByTag: Record<string, GQL.AudioMarkerDataFragment[]> = {};
